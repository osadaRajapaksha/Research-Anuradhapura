...
print("--- SCRIPT: BAYESIAN OPTIMIZED SMOTE + TUNED RANDOM FOREST ---")
...
print("3. Tuning SMOTE + Random Forest with Bayesian Optimization...")
pipeline = Pipeline([
...
search_space = {
    'rf__n_estimators':         Integer(100, 400),
...
test_df.to_csv('QGIS_Predictions_BO_SMOTE_Tuned_RF.csv', index=False)
print("Saved -> 'QGIS_Predictions_BO_SMOTE_Tuned_RF.csv'")
...
plt.title('Bayesian Optimized SMOTE + Tuned RF - Confusion Matrix')
plt.ylabel('True Reality (Map)')
plt.xlabel('Algorithm Prediction')
plt.tight_layout()
plt.savefig('Matrix_BO_SMOTE_Tuned_RF.png', dpi=300)
print("Saved -> 'Matrix_BO_SMOTE_Tuned_RF.png'")
